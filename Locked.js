	...
	self.setState({list:arr});
		...
		handleChecked(event){
		 ...
		 update_campaign(){
			var self=this;
			console.log('hi');
			axios.get("/api/campaigns/get_campaigns_locked",{params:{name:self.props.auth.user.name}}).then(function (response) {
	...
	deleteEntry= (event) => {
		...
		render(){
			...
			return(
				...
				<SideNav/>
				{/* {this.state.onClick ? this.state.onedit ? <Edit_Campaign selected={this.state.elem_details} />:<Execution_Sheet dash={this.state.campaign_name} />: */}
				{this.state.onClick ? this.state.onedit ? <LockedCampaign user={this.props.auth.user.name} backfun={this.backfun} selected={this.state.elem_details} update_campaign= {this.update_campaign}/>:<Execution_Sheet dash={this.state.campaign_name} />:
				<> 
		 ...
		 <span class="inner12 krbt" onClick={this.movetoPending} ><img src={MovetoIcon} className="icon-ht"/><span className=" simpletext theme-col" > Move to pending</span> </span> 
		 </span>
					</div>
 ...
 this.state.loader && <Home visible={this.state.loader} />
	...
	<Modal isOpen={this.state.isOpen} toggle={this.toggleModal}>
			...
			<ModalFooter>
				...
				<Button className="dash-button " onClick={this.deleteEntry}>Delete</Button>
			</ModalFooter>
			</Modal>
